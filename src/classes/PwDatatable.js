		...
		return {
			...
			loadTable() {
				var callback = (data_api) => {
					var { queryParams = {} } = this.getData();

					var { order_by, order, page, limit } = queryParams;
			...
			getDataApi(callback) {
				...
				var result = {};
				var { queryParams = {}, url } = this.getData();
			...
			countNbrPagination(data) {
				...
				if (
					data.totalFiltered &&
					parseInt(data.totalFiltered) != 0 &&
					parseInt(data.limit)
				) {
					result = Math.ceil(
						parseInt(data.totalFiltered) / parseInt(data.limit)
					);
				}
